...
const signupStudent = async (req, res, next) => {
  const { name, email, password, alumniID, dateOfBirth, address, contact } =
    req.body;
...
const loginStudent = async (req, res, next) => {
  ...
  try {
    const validUser = await Alumni.findOne({ alumniID });
    if (!validUser) {
      return next(errorHandler(404, "Alumni not found"));
    }

    const validPassword = bcryptjs.compareSync(password, validUser.password);
    if (!validPassword) {
      return next(errorHandler(404, "Invalid alumni password"));
    }
