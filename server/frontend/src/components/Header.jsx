...
export default function Header() {
  ...
  return (
    <Navbar className={`border-b-2 h-16 ${theme === 'light' ? 'bg-warmBeige text-darkSlate' : 'bg-jetBlack text-paleGray'} w-full`}>
      ...
      <Link to="/" className={`self-center whitespace-nowrap text-sm sm:text-xl font-semibold ${theme === 'light' ? 'text-darkSlate' : 'text-paleGray'}`}>
        <span className={`${theme === 'light' ? 'bg-rosewoodRed text-white' : 'bg-burntOrange text-black'} px-2 py-1 rounded-lg`}>HI</span>
        Alumni
      </Link>

    
      <Navbar.Collapse className="flex items-center space-x-4">
        <Navbar.Link active={path === "/"} as={'div'} className="flex items-center h-full">
          <Link to='/' >Home</Link>
        </Navbar.Link>
        <Navbar.Link active={path === "/officebearer"} as={'div'} className="flex items-center h-full">
          <Link to='/officebearer'>Profiles</Link>
        </Navbar.Link>
        <Navbar.Link active={path === "/jobs"} as={'div'} className="flex items-center h-full">
          <Link to='/jobs'>Jobs</Link>
        </Navbar.Link>
        <Navbar.Link active={path === "/event"} as={'div'} className="flex items-center h-full">
          <Link to='/event'>Events</Link>
        </Navbar.Link>
        <Navbar.Link active={path === "/gallery"} as={'div'} className="flex items-center h-full">
          <Link to='/gallery'>Blogs</Link>
        </Navbar.Link>
        <Navbar.Link active={path === "/contact"} as={'div'} className="flex items-center h-full">
          <Link to='/contact'>Query Box</Link>
        </Navbar.Link>
        <Navbar.Link active={path === "/about"} as={'div'} className="flex items-center h-full">
